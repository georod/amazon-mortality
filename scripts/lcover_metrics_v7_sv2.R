...
# This is a subversion to speed up processing
...
rv_match <- rv_match[rv_match$ryear %in% c(2012),]
...
foreach(j=1:4) %do% {
  ...
  foreach (m=1:length(rcropfiles)) %do% {
    ...
    path1 <- gsub("mun_rasters", 'mun_lcover', rv_match[j,"rcropf2"])
    
    dir.create(path=path1, recursive = TRUE)
    
    saveRDS(temp1, paste0(path1, "/", "lcm_", mun_code,".rds"))  # overwrite=TRUE?
    
    print(paste0("lcover metrics done. ", "year: " , yr_mun_code , " state: ", j, " mun poly :", mun_code))
    
    } else if (yr_mun_code  >= 2007 & yr_mun_code  < 2010) {
    
        mun_poly <- polyL2Pj[[j]][polyL2Pj[[j]]$"GEOCODIG_M"==mun_code,]
    #proj1 <- "+proj=poly +lat_0=0 +lon_0=-54 +x_0=5000000 +y_0=10000000 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs +type=crs"
    #mun_polyPj <- sf::st_transform(mun_poly, crs="EPSG:5641") # Brazil mercator
    mun_polyPj <- sf::st_transform(mun_poly, crs="EPSG:5880") # Brazil mercator
    #mun_polyPj <- sf::st_transform(mun_poly, crs=proj1) # Brazil mercator
    ...
    temp1 <- sample_lsm(rTemp1, mun_polyPj, plot_id=mun_polyPj$"GEOCODIG_M",
                        what = c("lsm_c_pland", "lsm_c_ca", "lsm_l_ta")) # "lsm_c_ca"
    ...
    } else if (yr_mun_code  >= 2010 & yr_mun_code  < 2013) {
    
     mun_poly <- polyL2Pj[[j]][polyL2Pj[[j]]$"CD_GEOCODM"==mun_code,]
    #proj1 <- "+proj=poly +lat_0=0 +lon_0=-54 +x_0=5000000 +y_0=10000000 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs +type=crs"
    #mun_polyPj <- sf::st_transform(mun_poly, crs="EPSG:5641") # Brazil mercator
    mun_polyPj <- sf::st_transform(mun_poly, crs="EPSG:5880") # Brazil mercator
    #mun_polyPj <- sf::st_transform(mun_poly, crs=proj1) # Brazil mercator
    ...
    temp1 <- sample_lsm(rTemp1, mun_polyPj, plot_id=mun_polyPj$"CD_GEOCODM",
                        what = c("lsm_c_pland", "lsm_c_ca", "lsm_l_ta")) # "lsm_c_ca"
  ...
  } else if (yr_mun_code  >= 2013 & yr_mun_code < 2019) {
  
  mun_poly <- polyL2Pj[[j]][polyL2Pj[[j]]$"CD_GEOCMU"==mun_code,]
    #proj1 <- "+proj=poly +lat_0=0 +lon_0=-54 +x_0=5000000 +y_0=10000000 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs +type=crs"
    #mun_polyPj <- sf::st_transform(mun_poly, crs="EPSG:5641") # Brazil mercator
    mun_polyPj <- sf::st_transform(mun_poly, crs="EPSG:5880") # Brazil mercator
    #mun_polyPj <- sf::st_transform(mun_poly, crs=proj1) # Brazil mercator
    ...
    temp1 <- sample_lsm(rTemp1, mun_polyPj, plot_id=mun_polyPj$"CD_GEOCMU",
                        what = c("lsm_c_pland", "lsm_c_ca", "lsm_l_ta")) # "lsm_c_ca"
  ...
  } else if ( yr_mun_code  >= 2019) {
  
  mun_poly <- polyL2Pj[[j]][polyL2Pj[[j]]$"CD_MUN"==mun_code,]
    #proj1 <- "+proj=poly +lat_0=0 +lon_0=-54 +x_0=5000000 +y_0=10000000 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs +type=crs"
    #mun_polyPj <- sf::st_transform(mun_poly, crs="EPSG:5641") # Brazil mercator
    mun_polyPj <- sf::st_transform(mun_poly, crs="EPSG:5880") # Brazil mercator
    #mun_polyPj <- sf::st_transform(mun_poly, crs=proj1) # Brazil mercator
    ...
    temp1 <- sample_lsm(rTemp1, mun_polyPj, plot_id=mun_polyPj$"CD_MUN",
                        what = c("lsm_c_pland", "lsm_c_ca", "lsm_l_ta")) # "lsm_c_ca"
