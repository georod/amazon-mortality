...
# This has to have 4326 project as as GEE raster are in this geo coor
#polyL2Pj2  <- foreach (i=1:nrow(rv_match)) %do% {
# polyL2Pj2  <- foreach (i=1:4) %do% {
#   
...
#foreach(j=1:length(polyL2Pj2), .packages=c('doParallel','foreach','terra', 'sf')) %do% {
#foreach(j=1:nrow(rv_match), .packages=c('doParallel','foreach','terra', 'sf')) %do% {
foreach(j=3:5, .packages=c('doParallel','foreach','terra', 'sf')) %do% {
  #foreach(j=2, .packages=c('doParallel','foreach','terra', 'sf')) %do% {
  ...
  #foreach (m=1:1) %do% {
    ...
    #mun_polyPj <- sf::st_transform(mun_poly, crs="EPSG:3857")
    ...
    #temp1 <- sample_lsm(rTemp0, mun_polyPj, plot_id=mun_polyPj$"munic",
     #                   what = c("lsm_c_pland", "lsm_c_ca", "lsm_l_ta")) # "lsm_c_ca"
